...
namespace Terratype.Indexer.Sql.Persistance.Context
{
	...
	{
		public void Write(string entryKey, Guid umbracoNode, Models.Model map, DateTime lastModified)
		{
			...
			if (entry == null)
			{
				...
				Database.Insert(nameof(Terratype) + nameof(Indexer) + nameof(Sql) + nameof(Data.Dto.Entry), nameof(Data.Dto.Entry.Identifier), false, entry);
			}
			else
			{
